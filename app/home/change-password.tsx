...
export default function ChangePasswordScreen() {
    ...
    const handleSave = () => {
        if (!currentPassword || !newPassword || !confirmPassword) {
            PlatformAlert.alert(labels.changePassword.errorTitle, labels.changePassword.emptyMessage);
            return;
        }

        if (newPassword !== confirmPassword) {
            PlatformAlert.alert(labels.changePassword.errorTitle, labels.changePassword.mismatchMessage);
            return;
        }

        if (newPassword.length < 6) {
            PlatformAlert.alert(labels.changePassword.errorTitle, labels.changePassword.lengthMessage);
            return;
        }

        // Mock Success
        PlatformAlert.alert(labels.changePassword.successTitle, labels.changePassword.successMessage, [
